...
> **Telegram-бот не запускается по умолчанию** — чтобы избежать конфликта с боевым ботом на сервере (два процесса на одном токене ломают друг друга). Если ты Артём и работаешь над ботом — используй тестового бота (@BotFather → новый бот) и запускай с профилем:
>
> ```bash
...
> **Напрямую в `main` не пушим.** Только через Pull Request с ревью Ивана.
...
# 3. Работать, коммитить (только свои папки!)
git add frontend/
...
#    base: main ← compare: имя/feat-описание
#    В ревьюеры назначить Ивана (@gubkinbot)

# 6. Дождаться ревью и одобрения от Ивана

# 7. После одобрения — Иван мержит PR

# 8. Обновить main локально
git checkout main
git pull
```
...
2. GitHub Actions автоматически деплоит на сервер (SSH → git pull → docker compose up)
3. Через ~1 минуту изменения на проде
4. Ветку можно удалить (GitHub предложит кнопку)
...
- **Тимур и Артём:** только через PR, ревьюер — Иван
- **Иван:** мелкие правки (README, конфиги, фиксы) — можно пушить в main напрямую. Крупные фичи в tablet/ — через PR, мержит сам
- **Одна фича — один PR.** Не мешай несколько задач в одном PR

## Деплой

Автоматический. Срабатывает **только при мерже в `main`**:
1. GitHub Actions подключается к серверу по SSH
2. Делает `git pull` + `docker compose --profile bot up --build -d`
3. Через ~1 минуту изменения на проде

**Не нужно** заходить на сервер вручную. Пуши в другие ветки деплой не вызывают.
...
```bash
# Запустить всё локально (без бота)
docker compose up --build

# Запустить с ботом (только Артём, с тестовым токеном!)
docker compose --profile bot up --build
